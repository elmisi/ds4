#!/usr/bin/env python3
"""Exact-gated, counterbalanced V4.1 prefill stage synchronization experiment.
...
def main():
    ...
    provenance = dict(source_root=str(root), source_commit=subprocess.check_output(
        ...
        plan=plan, reference=str(args.reference) if args.reference else None)
    timings = {0: [], 1: []}
    for label, enabled, dump in plan:
        ...
        command = [sys.executable, str(args.runner.resolve()), '--timeout', str(args.timeout),
                   ...
                   '--env', f'DS4_CUDA_V41_PREFILL_STAGE_SYNC={enabled}']
        if dump:
        ...
        print(json.dumps(dict(starting=name, stage_sync=enabled, dump=dump)), flush=True)
