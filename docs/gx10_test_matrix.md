...
| `hc_expand_soa_par_hc4` | `DS4_CUDA_HC_EXPAND_SOA_PAR_HC4=1` | lane-parallel n_hc=4 expansion/store after exact Q8 SoA dot, 15.91 vs 16.06 t/s control despite `REG:51` | diagnostic only; do not promote |
...
| `attn_a_direct_q8` | `DS4_CUDA_ATTENTION_OUTPUT_A_DIRECT_Q8=1` | direct `attn_output_a` -> Q8 blocks/scales for `attn_output_b` HC-expand was slower: 16.07 vs 16.15 with float low write, then 16.02 vs 16.13 after removing that write | diagnostic only; do not promote |
| `shape_gate_attn_qb_soa_b32` | `DS4_CUDA_MOE_DECODE_GATE_SHAPE2048_CONSTSTRIDE=1 DS4_CUDA_Q8_SOA_QB=1 DS4_CUDA_ATTN_Q_B_SOA_B32_SPECIAL=1` | composed signal: 16.21 vs 16.04 at 256 tokens, repeat 16.18 vs 15.85; selected tokens matched in a 32-step logprob check with ~7.7e-6 max diff, and 2q/4q `ds4-eval` smokes passed, but 12q no-think 2048 rejected it: `exact_fast` 12/12, combo 11/12, missed `AIME2025/aime2025-16` with `600` vs `468` after budget saturation | rejected for quality; do not promote or repeat exact combo |
...
| `indexer_qreuse2` | `DS4_CUDA_INDEXER_DIRECT_ONE_QREUSE2=1` | long-context indexer-score q-reuse: 13.52 vs 13.43 t/s at 64k | diagnostic only; below gate |
...
| `indexer_qreuse8` | `DS4_CUDA_INDEXER_DIRECT_ONE_QREUSE8=1` | best q-reuse row: 13.69 vs 13.56 at 64k; 12.73 vs 12.47 at 100k once; same-snapshot `soa` A/B still changed the 64-token greedy hash (`ba0b61c24114b3a4` -> `2c3d110fa4b36036`) | rejected for quality; do not promote |
