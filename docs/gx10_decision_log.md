...
| `indexer_qreuse2/4/8/16` / `DS4_CUDA_INDEXER_DIRECT_ONE_QREUSE*=1` | Long-context indexer-score q-reuse was speed-positive but rejected for quality: best row `qreuse8` measured 13.69 vs 13.56 at 64k and 12.73 vs 12.47 at 100k, but same-snapshot `soa` A/B changed the 64-token greedy hash (`ba0b61c24114b3a4` -> `2c3d110fa4b36036`). `qreuse16` was neutral. Reject; do not repeat row-count variants standalone. |
...
| Long-context indexer q-reuse row-count variants | `qreuse8` is speed-positive but same-snapshot greedy output changed; `qreuse16` shows the row-count family saturates. |
